...
int main(void)
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        if (!is_sorted(vi)) {
            std::cerr << "ERRORE: merge Sort ha fallito su un vettore di interi!\n";
            return EXIT_FAILURE; 
        }
    }
    ...
    if (!is_sorted(vs)) {
        std::cerr << "ERRORE: merge Sort ha fallito sul vettore di stringhe!\n";
        return EXIT_FAILURE; 
    }

    std::cout << "Test di merge Sort completati con successo!\n";
    return EXIT_SUCCESS; 
}